...
const DEFAULT_COLORS = ["#F6C750", "#E63525", "#E87D58"];
...
const generateColors = (name: string, colors = DEFAULT_COLORS) => {
    const hashCode = name.split('').reduce((acc: number, char: string) => {
...
const FallbackAvatar = ({ text="wanjohi", size = 40, colors = DEFAULT_COLORS }: Props) => {
    ...
    return (
        <svg
            ...
            height={size}
        >
            <title id={titleId}>Fallback avatar for {text}</title>
            <mask
                id="mask__marble"
          ...
          `}
            ...
            <defs>
                ...
                >
                    ...
                    <feGaussianBlur std-deviation={7} result="effect1_foregroundBlur" />
                </filter>
            </defs>
        </svg>
    );
...
export default qwikify$(FallbackAvatar);
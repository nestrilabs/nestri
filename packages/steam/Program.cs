...
// manager.Subscribe<SteamClient>(OnDisconnected);

manager.Subscribe<SteamUser.LoggedOnCallback>(OnLoggedOn);
manager.Subscribe<SteamUser.LoggedOffCallback>(OnLoggedOff);
manager.Subscribe<SteamUser.EmailAddrInfoCallback>(EmailAddressInfo);
manager.Subscribe<SteamUser.AccountInfoCallback>(AccountInfo);
...
async void OnConnected(SteamClient.ConnectedCallback callback)
{
    ...
    Console.WriteLine($"Refresh Token is '{pollResponse.RefreshToken}' ");
...
void EmailAddressInfo(SteamUser.EmailAddrInfoCallback callback)
{
    Console.WriteLine("Email Address for this Account: {0}", callback.EmailAddress);
}

void AccountInfo(SteamUser.AccountInfoCallback callback)
{
    Console.WriteLine("Persona Name for this Account: {0}", callback.PersonaName);
}
...
void DrawQRCode(QrAuthSession authSession)
{
    ...
    // Console.WriteLine( "ChallengeUrl", authSession.ChallengeURL);
}